...
import { BehaviorSubject, catchError, forkJoin, map, of, exhaustMap, finalize } from 'rxjs';
...
export class PortfoliosPageFacade {
  ...
  // Using exhaustMap to prevent cancelling in-flight requests
  private portfolios$ = this.loadTrigger$.pipe(
    exhaustMap(() => {
      this.loadingState.set(true);

      return this.portfolioApiService.getPortfolios().pipe(
        exhaustMap((portfolios) => this.enrichPortfolios(portfolios)),
        catchError((error) => {
  ...
  private enrichPortfolios(portfolios: DashboardPortfolio[]) {
    ...
    const summaryRequests = portfolios.map((portfolio) =>
      this.portfolioApiService.getPortfolioSummary(portfolio.id).pipe(
        ...
        catchError((error) => {
          console.warn(
            `Failed to fetch summary for portfolio ${portfolio.id}:`,
            error
          );
  ...
  private toPortfolioCardData(
    ...
    if (summary) {
      ...
      const nonCashPositions =
        summary.positions?.filter((p: any) => p.ticker !== 'CASH') || [];
