...
export class GrokLlmService {
     ...
     */
    private mapUsage(usage?: any): GrokUsageMetadata {
        return {
    ...
    async generateContent(prompt: string, model?: string): Promise<GrokResponse> {
        ...
        throw new Error(
            lastError?.message || 'Unknown error during Grok API call',
        );
    }
            ...
            if (result.sources) {
                for (const source of result.sources) {
                    if (typeof source === 'string') {
                        ...
                        sources.push((source as any).url);
                    } else if (source && typeof source === 'object' && 'id' in source) {
                        sources.push((source as any).id);
                    }
                }
            }
