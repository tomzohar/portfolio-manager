...
export class ChartDataService {
   ...
   * Uses Time-Weighted Return (TWR) formula on invested equity only
   *
   * Why: Allows users to see performance of their investment decisions
   * separate from cash drag effects
   */
  private calculateDailyReturnExcludingCash(
    ...
    if (denominator === 0 && endEquity === 0) {
      ...
      return 0; // Starting with zero invested capital
    }

    return (endEquity - startEquity - netCashFlow) / denominator;
  }
